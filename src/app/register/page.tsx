  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-[#050505] text-slate-100 px-4 overflow-hidden selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute -top-[30%] right-[10%] w-[90vw] h-[90vw] md:w-[60vw] md:h-[60vw] rounded-full bg-[radial-gradient(circle,rgba(99,102,241,0.12)_0%,transparent_70%)] blur-[95px]" />
        <div className="absolute bottom-[10%] -left-[10%] w-[80vw] h-[80vw] md:w-[50vw] md:h-[50vw] rounded-full bg-[radial-gradient(circle,rgba(168,85,247,0.08)_0%,transparent_70%)] blur-[90px]" />
      </div>
        ...
        <div className="bg-slate-950/80 border border-white/5 shadow-[inset_0_1px_1px_rgba(255,255,255,0.15)] rounded-[calc(2.5rem-0.5rem)] p-8 md:p-10 flex flex-col gap-6">
          ...
          {error && (
            <div className="p-4 rounded-2xl border border-red-500/20 bg-red-500/10 text-red-400 text-sm flex items-start gap-2.5 animate-in fade-in duration-200">
              ...
              <div className="flex-1 font-medium">
                <span className="font-bold block mb-0.5">Ada masalah saat mendaftar</span>
                <span>{error}</span>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <label htmlFor="companyName" className="text-[10px] font-bold text-slate-400 tracking-wider uppercase block">
                Nama Perusahaan / EO
              </label>
              <input
                id="companyName"
                ...
                className="w-full px-4 py-3 rounded-full border border-white/10 bg-slate-950/80 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 transition duration-300 text-sm font-medium"
              />
            </div>
